...
{
    public partial class HandRecognition : Form
    {
        public delegate void UpdatePlayerScore(string Score);
        public event UpdatePlayerScore UpdateScore;

       // public delegate void Update_(string Score);
       // public event Update_ update;
        ...
        private void DrawAndComputeFingersNum()
        {
            try
            {
                ...
                if (fingerNum != 0)
                    Repeated.Add(fingerNum);

                if (Repeated.Count >= 200)
                {

                    var maxRepeated = Repeated.GroupBy(s => s)
                         .OrderByDescending(s => s.Count())
                         .First().Key;
                    UpdateScore(Convert.ToString(maxRepeated));

                   // update(Convert.ToString(maxRepeated));

                    File.AppendAllText(@"E:\HandGestureRecognition.txt", "\n Detected: " + maxRepeated.ToString());
